...
def send_file(client_socket, file_path, recipient):
    ...
    message = f"!file {file_type} {file_size} {recipient}"
    client_socket.send(message.encode("utf-8"))

    # Kirim nama file bersama dengan data file
...
def receive_file(client_socket, file_path, file_size):
...
def main():
    ...
    while True:
        ...
        if choice == "1":
            ...
            send_file(client_socket, file_path, send_to)
            continue
        elif choice == "5":
            file_name = input("Masukkan nama file yang akan diterima: ")
            file_size = int(input("Masukkan ukuran file yang akan diterima: "))
